...
class PostUpdateDelete(APITestCase):
    ...
    def setUp(self):
        ...
        self.password = "test1234"
        self.user = User.objects.create_user(username=self.username, password=self.password)
        self.user2 = User.objects.create_user(username="selcuk3", password=self.password)
    ...
    def test_jwt_authentication(self, username="selcuk", password="test1234"):
    ...
    def test_post_delete_different_user(self):
        self.test_jwt_authentication("selcuk3")
        response = self.client.delete(self.url)
